...
export default function CustomTour() {
  ...
  async function handleSubmit(e) {
    ...
    if (!destinationsStr || !month || !name || !email || !phone) {
      setError('Please select at least one country, your name, email, phone number and travel month.')
      return
    }
    setError('')
    setWaking(false)
    ...
    try {
      ...
      navigate('/thank-you', {
        state: {
          ...
          name,
          enquiryLabel: 'custom tour',
          whatsappText: `Hi, I just submitted a custom tour enquiry.\n\nDestinations: ${destinationsStr}\nName: ${name}\nEmail: ${email}\nPhone: ${phone}\nMonth: ${month} 2026\nDuration: ${duration} days\nGroup: ${groupSize} person${groupSize > 1 ? 's' : ''}${car ? `\nCar: ${car}` : ''}\nBudget: €${budget.toLocaleString()}${travelDatesStr ? `\nDates: ${travelDatesStr}` : ''}${notes ? `\nNotes: ${notes}` : ''}`,
          backTo: '/destinations',
          backLabel: 'Explore destinations',
          details: [
            { label: 'Destinations', value: destinationsStr },
