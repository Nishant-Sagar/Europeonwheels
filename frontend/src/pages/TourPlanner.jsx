  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      navigate('/thank-you', {
        state: {
          lead: true,
          formType: 'single-country',
          name,
          enquiryLabel: `${data.name} trip`,
          whatsappText: `Hi, I just submitted a tour enquiry for ${data.name}.\n\nName: ${name}\nEmail: ${email}\nPhone: ${phone}\nMonth: ${month} 2026\nDuration: ${duration} days\nGroup: ${groupSize} person${groupSize > 1 ? 's' : ''}${car ? `\nCar: ${car}` : ''}\nBudget: €${budget.toLocaleString()}${notes ? `\nNotes: ${notes}` : ''}`,
          backTo: '/single-country',
          backLabel: 'Explore other countries',
          details: [
            { label: 'Destination', value: data.name },
...
{/* Month */}
              ...
              <button type="submit" disabled={submitting}
                ...
                {submitting ? (waking ? "Almost there \u2014 waking up our server\u2026" : "Sending your enquiry...") : "Save & send my " + data.name + " trip enquiry \u2192"}
              </button>
